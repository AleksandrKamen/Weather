package model.users.repository;
...
public class UserRepository extends BaseRepository<UsersEntity,Long> {
   public UserRepository(){super(UsersEntity.class);}

   public Optional<UsersEntity> findByLogin(String login){
       try (var session = HibernateUtil.getSession()) {
           session.beginTransaction();
           var query = session.createQuery("select u from UsersEntity u LEFT JOIN FETCH u.sessions where u.login = :login", UsersEntity.class);
